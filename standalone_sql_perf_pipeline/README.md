...
Set the default with the `ACTIVE_ITERATION = …` block at the top of `main.py`, or override without editing:
...
| `-n` | Name | What runs |
| ---: | --- | --- |
| **1** | Global classifier | Full pipeline: load/time BIRD → features → labels → **database-aware** train/test (`HOLDOUT_DATABASES`) → CV + test evaluation → `data/*.csv`, `artifacts/best_model.joblib`, `reports/*`. |
| **2** | Commit / geometry comparison | `rerun_commit_comparison.main()`: checks out fixed commits into worktrees, recomputes **seen** (per-DB 80/20 pooled) vs **unseen** (holdout-DB) metrics; writes `commit_rerun_metrics.csv` and related CSVs. Requires **git** and disk for `.tmp_commit_eval/`. |
| **3** | Per-DB logistic | `run_within_db_logistic.py` → `reports/within_db_logistic_metrics.csv`. |
| **4** | Per-DB regression + schema | `run_schema_stats_model.py` → `reports/within_db_schema_metrics.csv`. |
| **5** | Cross-schema regression | `run_full_regression.py` (stdout; bundle builder can log it). |
| **6** | Figures 13–14 | `run_phase6_figures.py` (needs iteration 4 output). |
| **7** | Narrative graphs | `run_report_graphs.py`. |
...
0. **Bundled CSV snapshot** — `data/query_dataset_raw.csv` and `data/query_dataset_features.csv` are **copies** of the parent project outputs so you can run downstream scripts immediately. Replace them by running `python main.py -n 1` after refreshing BIRD timing. See `data/README.md`.

1. **BIRD Mini-Dev** under one of the layouts `config.py` searches (typical clone):
...
2. Create empty dirs on first run (or let the pipeline create them):
